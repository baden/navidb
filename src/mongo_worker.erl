...
%% ----------------------------------------------------------------------------
-export([
    insert/2,           %% insert the records
    update/2,           %% update by new Doc
    update/3,           %% update by match with new doc
    update/5,           %% update by match with new doc, with args
...
-spec insert(binary(), any()) -> {ok, any()} | {error, any()}.
insert(Coll, Doc) ->
  gen_server:call(?MODULE, {insert, Coll, Doc}).
...
-spec update(binary(), any(), any(), boolean(), boolean()) -> {ok, any()} | {error, any()}.
update(Coll, Match, Doc, Upsert, MultiUpdate) when is_map(Doc) ->
  gen_server:call(?MODULE, {update, Coll, Match, Doc, Upsert, MultiUpdate}).
...
-spec aggregate(binary(), list()) -> {ok, any()}.
aggregate(Coll, Pipeline) ->
  gen_server:call(?MODULE, {aggregate, Coll, Pipeline}).
...
handle_call({insert, Coll, Doc}, _From, #state{pool=Pool} = State) ->
...
handle_call({update, Coll, Match, Doc, Upsert, MultiUpdate}, _From, #state{pool=Pool} = State) ->
    Reply = poolboy:transaction(Pool,
            fun(Conn) ->
                case catch mc_worker_api:update(Conn, Coll, Match, Doc, Upsert, MultiUpdate) of
                    {'EXIT', Error} -> {error, Error};
                    Else -> {ok, Else}
                end
            end),
...
handle_call({aggregate, Coll, Pipeline}, _From, #state{pool=Pool} = State) ->
    Res = poolboy:transaction(Pool,
            fun(Conn) ->
